...
object FuelGenerator : Worker {
    ...
    private fun addBurnTime(world: World, furnacePos: BlockPos) {
        ...
        if (blockEntity is AbstractFurnaceBlockEntity) {
            ...
            val addedBurnTime = config.burnTimeSeconds * 20
            val currentBurnTime = accessor.getBurnTime()
            accessor.setBurnTime(currentBurnTime + addedBurnTime)
            accessor.setFuelTime(currentBurnTime + addedBurnTime)
